...
export default function SikareadsClient({ userId, userEmail, books, progress, purchases }: Props) {
  ...
  // Auto-add purchased books that are not yet tracked
  useEffect(() => {
  ...
  const updateStatus = useCallback(async (book: Book, status: "want" | "reading" | "done") => {
    setUpdating(book.id);
    try {
      const supabase = (await import("../../lib/supabase")).createClient();
      ...
      if (existing) {
        ...
        const { data } = await supabase.from("reading_progress")
          .insert({ user_id: userId, book_id: book.id, status, progress_pct: status === "done" ? 100 : 0, ...(status === "reading" ? { started_at: new Date().toISOString() } : {}), ...(status === "done" ? { finished_at: new Date().toISOString() } : {}) })
          .select("*, books(*)").single();
        if (data) setMyProgress(prev => [...prev, data]);
      }
    } catch (e) { console.error(e); }
  ...
  return (
    <div style={{ padding: "40px 48px" }}>
      ...
      <div className="dash-cards" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "24px" }}>
        ...
        <div style={{ background: "#141414", border: "1px solid #1e1e1e", borderRadius: "16px", padding: "24px" }}>
          <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "10px", color: "#7a7468", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "10px" }}>Purchased</div>
          <div style={{ fontFamily: "Fraunces, serif", fontSize: "52px", fontWeight: 300, color: "#f4ecd8", letterSpacing: "-0.03em", lineHeight: 1, marginBottom: "8px" }}>{purchases.length}</div>
          <div style={{ fontSize: "12px", color: "#7a7468" }}>From sikareads.com</div>
        </div>
      </div>
            ...
            <div key={p.id} style={{ display: "flex", gap: "16px", alignItems: "flex-start", marginBottom: "16px" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
                  ...
                  <button onClick={() => updateStatus(p.books, "done")} style={{ padding: "4px 10px", background: "#0a0a0a", border: "1px solid #7aa87a", borderRadius: "100px", fontSize: "10px", color: "#7aa87a", cursor: "pointer" }}>Finished</button>
                </div>
              </div>
            </div>
          ))}
      ...
      <div style={{ background: "#141414", border: "1px solid #1e1e1e", borderRadius: "16px", padding: "22px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "16px" }}>
          {filteredBooks.map(book => {
            ...
            return (
              <div key={book.id} style={{ background: "#0a0a0a", border: "1px solid #2a2a2a", borderRadius: "12px", padding: "16px", display: "flex", flexDirection: "column", gap: "10px", transition: "border-color 0.2s", cursor: "pointer" }}
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                    {book.pillar_tag && (
                      <span style={{ fontSize: "9px", color: PILLAR_COLORS[book.pillar_tag] || "#7a7468", padding: "2px 6px", border: "1px solid", borderColor: PILLAR_COLORS[book.pillar_tag] || "#7a7468", borderRadius: "100px" }}>{book.pillar_tag}</span>
                    )}
                    {isPurchased && (
                      <span style={{ fontSize: "9px", color: "#7aa87a", padding: "2px 6px", border: "1px solid #7aa87a", borderRadius: "100px", background: "rgba(122,168,122,0.1)" }}>✓ Purchased</span>
                    )}
                  </div>
                </div>
...
console.log('Done: SikareadsClient.tsx rewritten cleanly');