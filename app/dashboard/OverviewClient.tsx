...
type Goal = { id: string; name: string; target: number; saved: number; deadline: string; category: string; };
...
type Debt = { id: string; name: string; balance: number; monthly: number; };
...
export default function OverviewClient({ profile, transactions, goals, assets, liabilities, debts, month }: Props) {
  ...
  // Net worth
  const totalAssets = assets.reduce((s, a) => s + (a.value * (FX[a.currency] || 1)), 0);
  const totalLiabilities = liabilities.reduce((s, l) => s + l.value, 0);
  const netWorth = totalAssets - totalLiabilities;

  // Retirement
  const workingYears = profile ? Math.max(0, profile.retire_age - profile.age) : 0;
  const annualNeed = (profile?.monthly_income || 0) * 12 * 0.7;
  const corpus = annualNeed * 25;
  const monthlyRate = 0.08 / 12;
  const months = workingYears * 12;
  const currentFV = (profile?.current_savings || 0) * Math.pow(1 + monthlyRate, months);
  ...
  const howFarScore = financeScore;
  ...
  const getFocus = () => {
    ...
    return { text: `Close the gap on your ${closest.name}. About GHS ${fmt(needed)}/month gets you there on time.`, action: "Update goal", href: "/dashboard/goals" };
  };
  ...
  return (
    <div style={{ padding: "40px 48px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "32px", paddingBottom: "20px", borderBottom: "1px solid #1e1e1e", flexWrap: "wrap", gap: "16px" }}>
        <div>
          <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "#d4a947", letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: "8px" }}>{today} · Accra</div>
      ...
      {/* Top row - score, retirement, net worth */}
      <div className="dash-cards" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr", gap: "16px", marginBottom: "16px" }}>
        ...
        <div style={{ background: "radial-gradient(circle at 90% 10%, rgba(212,169,71,0.12) 0%, transparent 40%), #141414", border: "1px solid #2a2a2a", borderRadius: "16px", padding: "28px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", paddingTop: "16px", borderTop: "1px solid #1e1e1e" }}>
            ...
            <div>
              <div style={{ fontSize: "10px", color: "#7a7468", letterSpacing: "0.08em", textTransform: "uppercase" }}>Savings rate</div>
              <div style={{ fontFamily: "Fraunces, serif", fontSize: "20px", color: "#f4ecd8", marginTop: "4px" }}>{Math.max(0, savingsRate)}<em style={{ color: "#d4a947", fontStyle: "italic", fontSize: "14px" }}>%</em></div>
            </div>
          </div>
        </div>
        ...
        <div style={cardStyle()}>
          ...
          {goals.length === 0 ? (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              <div style={{ fontSize: "13px", color: "#7a7468", marginBottom: "12px" }}>No goals yet. Goals turn dreams into deadlines.</div>
